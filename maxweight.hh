...
)
{
	...
	for (size_t i = 0; i < source.size(); i++)
	{
		...
		if (source[i]->weight() >= min_weight && source[i]->weight() <= max_weight)
		{
			...
			volume_cubic = source[i]->volume();
			weight_TEU = source[i]->weight();
			result->push_back(std::shared_ptr<CargoItem>(new CargoItem(description, volume_cubic, weight_TEU)));
			num_of_cargo++;
		}
		if (num_of_cargo == total_size)
			break;
	}
...
)
{
	...
	while (!todo.empty())
	{
		double greatest = todo[0]->weight() / todo[0]->volume();
		size_t greatest_index = 0;

		for (size_t i = 0; i < todo.size(); i++)
		{
			if (greatest < (todo[i]->weight() / todo[i]->volume()))
			{
				greatest = (todo[i]->weight() / todo[i]->volume());
				greatest_index = i;
			}
		}
		double c = todo[greatest_index]->volume();
		auto a = todo[greatest_index];
		//todo.erase(todo.begin() + greatest_index); // Erase a from todo

		if ((result_volume + c) <= total_volume)
		{
			result->push_back(a);
			result_volume += c;
		}
		todo.erase(todo.begin() + greatest_index);
	}
	return result;
}
		...
		if (candidate_volume <= total_volume)
		{
			if (best->empty() || candidate_weight > best_weight)
			{
				...
				for (size_t i = 0; i < candidate->size(); i++)
				{
					...
					best_volume += candidate->at(i)->volume();
					best_weight += candidate->at(i)->weight();
				}
			}
		}
	}
